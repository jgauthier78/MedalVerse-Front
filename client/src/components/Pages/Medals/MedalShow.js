...
function Anim() {
    ...
    const _gesture = ({ args: [index], down, delta: [xDelta], distance, direction: [xDir], velocity }) => {
    ...
    return props.map(({ x, y, rot, scale }, i) => (
        ...
        <animated.div className="MedalShow-Container" key={i} style={{ transform: interpolate([x, y], (x, y) => `translate3d(${x}px,${y}px,0)`) }}>
            ...
            <animated.div className="MedalShow-Stack" {...bind(i)} style={{ transform: interpolate([rot, scale], trans), backgroundImage: `url(${cards[i]})` }} />
        </animated.div>
    ))
}
